...
/**
  ...
  MEM64 resources are promoted to PMEM64 to enable 64-bit allocation
  ...
  )
{
  ...
  if (sizeof (UINTN) <= 4) {
    ...
    // promote to PMEM64 first, instead of degrading to MEM32 directly.
    //
    ...
    if (!BridgeSupportResourceDecode (Bridge, EFI_BRIDGE_MEM64_DECODE_SUPPORTED)) {
      if (FeaturePcdGet (PcdPciDegradeMem64toPMem64) &&
