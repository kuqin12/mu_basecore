...
extern "C" {
  #include "../../Misc/MemoryProtectionSupport.h"
...
TEST_F (SplitIncomingRangeTest, AddRangeOverlapSingleBin) {
  ...
  EXPECT_EQ (MemDesc[1].Type, EfiBootServicesData);
...
TEST_F (SplitIncomingRangeTest, AddRangeWithinSingleBin) {
  ...
  ASSERT_FALSE (EFI_ERROR (Status));
  ASSERT_EQ (MemDescCount, (UINTN)1);
  EXPECT_EQ (MemDesc[0].Type, EfiACPIReclaimMemory);
  EXPECT_EQ (MemDesc[0].PhysicalStart, (EFI_PHYSICAL_ADDRESS)(SIZE_64KB + EFI_PAGE_SIZE));
  EXPECT_EQ (MemDesc[0].NumberOfPages, (UINT64)1);
  EXPECT_EQ (MemDesc[0].Attribute, (UINT64)(EFI_MEMORY_UC | EFI_MEMORY_WP));
}
...
TEST_F (SplitIncomingRangeTest, AddRangeCoverSingleBinFromStart) {
  ...
  ASSERT_FALSE (EFI_ERROR (Status));
  ASSERT_EQ (MemDescCount, (UINTN)2);
  EXPECT_EQ (MemDesc[0].Type, EfiBootServicesData);
  EXPECT_EQ (MemDesc[0].PhysicalStart, (EFI_PHYSICAL_ADDRESS)(SIZE_64KB - EFI_PAGE_SIZE));
  EXPECT_EQ (MemDesc[0].NumberOfPages, (UINT64)1);
  EXPECT_EQ (MemDesc[0].Attribute, (UINT64)(EFI_MEMORY_UC | EFI_MEMORY_WP));
...
TEST_F (SplitIncomingRangeTest, AddRangeOverlapMultipleBins) {
  CONST UINTN               MaxMemDescCount = 10;
  EFI_MEMORY_DESCRIPTOR     MemDesc[MaxMemDescCount];
  UINTN                     MemDescCount = MaxMemDescCount;
  EFI_STATUS                Status;
  ...
  ASSERT_FALSE (EFI_ERROR (Status));
  ASSERT_EQ (MemDescCount, (UINTN)5);
  EXPECT_EQ (MemDesc[0].Type, EfiBootServicesData);
  EXPECT_EQ (MemDesc[0].PhysicalStart, (EFI_PHYSICAL_ADDRESS)(SIZE_64KB - EFI_PAGE_SIZE));
  EXPECT_EQ (MemDesc[0].NumberOfPages, (UINT64)1);
  EXPECT_EQ (MemDesc[0].Attribute, (UINT64)(EFI_MEMORY_UC | EFI_MEMORY_WP));
  ...
  EXPECT_EQ (MemDesc[2].Type, EfiBootServicesData);
  ...
  EXPECT_EQ (MemDesc[3].Type, EfiBootServicesData);
  ...
  EXPECT_EQ (MemDesc[4].Type, EfiBootServicesData);
...
// existing ranges, starting from the first special bin.
TEST_F (SplitIncomingRangeTest, AddRangeCoverMultipleBinsFromStart) {
  ...
  // This range covers EfiACPIReclaimMemory, EfiACPIMemoryNVS,
  // EfiMemoryMappedIO and EfiMemoryMappedIOPortSpace
  Status = SplitIncomingRange (
             EfiBootServicesData,
             SIZE_64KB,
...
TEST_F (SplitIncomingRangeTest, AddRangeCoverAllBinsWithExtraSpace) {
  CONST UINTN               MaxMemDescCount = 20;
  EFI_MEMORY_DESCRIPTOR     MemDesc[MaxMemDescCount];
  UINTN                     MemDescCount = MaxMemDescCount;
  EFI_STATUS                Status;
  ...
  for (UINTN Index = 1; Index < MemDescCount - 2; Index+=2) {
    ...
    EXPECT_EQ (MemDesc[Index + 1].Type, EfiBootServicesData);
  ...
  EXPECT_EQ (MemDesc[MemDescCount - 2].Type, EfiBootServicesData);
  EXPECT_EQ (MemDesc[MemDescCount - 2].PhysicalStart, (EFI_PHYSICAL_ADDRESS)SIZE_64KB * (MemDescCount / 2));
  EXPECT_EQ (MemDesc[MemDescCount - 2].NumberOfPages, (UINT64)1);
  EXPECT_EQ (MemDesc[MemDescCount - 2].Attribute, (UINT64)(EFI_MEMORY_UC | EFI_MEMORY_WP));

  EXPECT_EQ (MemDesc[MemDescCount - 1].Type, EfiBootServicesData);
  EXPECT_EQ (MemDesc[MemDescCount - 1].PhysicalStart, (EFI_PHYSICAL_ADDRESS)(SIZE_64KB * (MemDescCount / 2) + EFI_PAGE_SIZE));
  EXPECT_EQ (MemDesc[MemDescCount - 1].NumberOfPages, (UINT64)31);
...
TEST_F (SplitIncomingRangeTest, AddRangeCoverAllBinsWithExtraSpaceAtBeginning) {
  ...
  EXPECT_EQ (MemDesc[MemDescCount - 1].Type, EfiBootServicesData);
  EXPECT_EQ (MemDesc[MemDescCount - 1].PhysicalStart, (EFI_PHYSICAL_ADDRESS)(SIZE_64KB * (MemDescCount / 2) + EFI_PAGE_SIZE));
  EXPECT_EQ (MemDesc[MemDescCount - 1].NumberOfPages, (UINT64)79);
  EXPECT_EQ (MemDesc[MemDescCount - 1].Attribute, (UINT64)(EFI_MEMORY_UC | EFI_MEMORY_WP));
}
