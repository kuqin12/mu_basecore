...
protected:
    ...
    ) override
  {
    ...
    // Inspect the mMemoryTypeStatisticsSortedByAddress array to set up the bins
    for (UINTN Index = 0; Index <= EfiMaxMemoryType; Index++) {
      if (!mMemoryTypeStatisticsSortedByAddress[Index].Special) {
        continue;
      }
...
TEST_F (CoreGetMemoryMapTest, AllocateSpecialRange) {
  VOID *RtBuffer;
  UINTN MemoryMapSize;
  EFI_MEMORY_DESCRIPTOR MemoryMap[20];
  ...
  while (EfiMemNext < EfiMemoryMapEnd) {
    ...
    ASSERT_LE (EfiMemNext, EfiMemoryMapEnd);
  }

  // Make sure all special memory types have been verified
  for (Index = 0; Index <= EfiMaxMemoryType; Index++) {
    if (mMemoryTypeStatisticsSortedByAddress[Index].Special) {
      ASSERT_EQ (mMemoryTypeStatistics[mMemoryTypeStatisticsSortedByAddress[Index].InformationIndex
        ].NumberOfPages, (UINTN)0);
    }
  }
