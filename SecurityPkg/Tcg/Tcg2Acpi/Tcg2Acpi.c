...
{
  EFI_STATUS                                Status;
  EFI_MM_COMMUNICATION_PROTOCOL             *MmCommunication = NULL;
  ...
  CommBuffer->Function = TPM_NVS_MM_EXCHANGE_INFO;
  CommBuffer->TargetAddress = (EFI_PHYSICAL_ADDRESS) TcgNvs;
  ...
  if (!EFI_ERROR (CommBuffer->ReturnStatus)) {
    ...
    TcgNvs->PhysicalPresence.SoftwareSmi = (UINT8) CommBuffer->TargetValue;
  }

  return CommBuffer->ReturnStatus;
}
  ...
  )
{
  ...
  TcgNvs = AssignOpRegion (Table, SIGNATURE_32 ('T', 'N', 'V', 'S'), (UINT16) sizeof (TCG_NVS));
  ASSERT (TcgNvs != NULL);
  TcgNvs->TpmIrqNum            = PcdGet32(PcdTpm2CurrentIrqNum);
  TcgNvs->IsShortFormPkgLength = IsShortFormPkgLength;

  Status = ExchangeCommonBuffer (TcgNvs);
