  ...
  )
{
  ...
  switch (CommParams->Function) {
    case TPM_NVS_MM_EXCHANGE_INFO:
      DEBUG ((DEBUG_VERBOSE, "[%a] - Function requested: MM_EXCHANGE_NVS_INFO\n", __FUNCTION__));
      CommParams->TargetValue = PpSoftwareSmi;
      mTcgNvs = (TCG_NVS*) CommParams->TargetAddress;
      break;
...
)
{
  ...
  if (ReadyToLockHandle != NULL) {
    Status = gMmst->MmiHandlerUnRegister (ReadyToLockHandle);
    ReadyToLockHandle = NULL;
  }
  return Status;
}
  ...
  )
{
  ...
  ReadyToLockHandle = NULL;
  Status = gMmst->MmiHandlerRegister (TpmNvsCommunciate, &gTpmNvsMmGuid, &ReadyToLockHandle);
  if (EFI_ERROR (Status)) {
  ...
  PpSoftwareSmi = (UINT8) SwContext.SwSmiInputValue;
